#!/usr/bin/env python3
"""
entropy_v4_regime.py — Strategy v4 with market regime detection

REGIMES:
  TRENDING (ADX > 25):     Trade BOS/CHoCH continuation, skip mean reversion
  RANGING (ADX < 20):       Trade STDV+OTE mean reversion (fade sweeps)
  CHOPPY (ADX 20-25):      No trades (uncertain)
  BREAKOUT (volatility > 2x avg): Reduce size, widen SL

Uses 14-bar ADX on M5 for regime classification.
...
def calc_adx(bars: List[Dict], period: int) -> float:
    ...
    for i in range(period, len(tr_list)):
        atr = tr_list[i] * k + atr * (1 - k)
        plus_di = plus_dm_list[i] * k + plus_di * (1 - k)
        minus_di = minus_dm_list[i] * k + minus_di * (1 - k)
    
    if atr > 0:
        plus_dx = 100 * plus_di / atr
        minus_dx = 100 * minus_di / atr
        dx = abs(plus_dx - minus_dx) / (plus_dx + minus_dx + 0.0001) * 100
        return dx
    return 0.0
...
def get_regime(bars: List[Dict], idx: int) -> str:
    """Returns: 'trending', 'ranging', or 'choppy'."""
    adx_window = bars[max(0, idx - ADX_PERIOD * 2) : idx + 1]
    adx = calc_adx(adx_window, ADX_PERIOD)
...
def run_backtest(df) -> Dict:
    ...
    regime_counts = {"trending": 0, "ranging": 0, "choppy": 0}
    ...
    for i in range(20 + 2, len(bars) - 2):
        ...
        # Regime detection
        regime = get_regime(bars, i)
        regime_counts[regime] += 1
        
        # Only trade in ranging markets (mean reversion works here)
        if regime != "ranging":
            continue
        ...
        trades.append({
            ...
            "equity": round(equity, 2), "trend": trend, "regime": regime,
        })
...
if __name__ == "__main__":
    ...
    print(f"\n{'='*70}")
    print(f" ENTROPY STDV+OTE — v4 WITH REGIME DETECTION")
    print(f"{'='*70}")
    ...
    print(f"\n  Regime distribution:")
    for r, c in result['regime_counts'].items():
        print(f"    {r}: {c} bars ({c/result['bars']*100:.1f}%)")

    out = "/Users/yuhfriendchris/Omni-full-ALGO-Trading-Bot/python/entropy_v4_honest.json"
    ...
    if result["detail"]:
        ...
        for t in result["detail"][:3]:
            print(f"   {t['time']} {t['type'].upper()} Regime={t['regime']} E={t['entry']:.2f} SL={t['sl']:.2f} TP={t['tp']:.2f} → {t['reason']} ${t['pnl']:.2f}")
        print(f"\n📊 Last 3:")
        for t in result["detail"][-3:]:
            print(f"   {t['time']} {t['type'].upper()} Regime={t['regime']} E={t['entry']:.2f} SL={t['sl']:.2f} TP={t['tp']:.2f} → {t['reason']} ${t['pnl']:.2f}")
