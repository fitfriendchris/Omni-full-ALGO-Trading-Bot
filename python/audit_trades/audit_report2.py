...
print('=== AUDIT: ACTUAL TOP 5 WINNING SEEDS (TRADE-BY-TRADE) ===\n')
files = sorted(glob.glob('seed_*_trades.csv'))
# Order by seed: 193, 171, 877, 695, 689
order = ['seed_193', 'seed_171', 'seed_877', 'seed_695', 'seed_689']
for key in order:
    f = next((x for x in files if key in x), None)
    if not f: continue
    df = pd.read_csv(f)
    seed = f.split('_')[1]
    ...
    print('Equity milestones (compounding path):')
    equity = 100.0
    eq_pts = [100.0]
